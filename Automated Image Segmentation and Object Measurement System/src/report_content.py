...
The report is written once here, as a sequence of neutral blocks, and rendered
twice: make_report.py turns the blocks into the PDF and make_report_docx.py
turns the same blocks into a Word document. Keeping a single source for the
content means that the two versions of the report can never drift apart.

Every block is a tuple whose first element names its kind:

    ("h1" | "h2" | "h3", text)
    ("p", text, style)              style is one of the names in STYLES below
    ("bullets", [text, ...])
    ("table", header, rows, col_widths_cm, font_size, highlight_rows)
    ("caption", text)
    ("figure", filename, caption, max_height_cm)
    ("pre", text)                   fixed pitch block, for example a command list
    ("code_heading", text), ("code", text)
    ("rule", width_cm, colour, thickness, space_before, space_after)
    ("spacer", points), ("page_break",)

Light inline markup is allowed inside the text of a paragraph: <b>bold</b>,
<font face='Courier'>fixed pitch</font>, and the entities &bull; &nbsp;
&middot; &amp; &lt; &gt;.
"""
...
# Which renderer is building the document. make_report_docx.py sets this to
# "docx" before it imports this module, so that the contents table can carry
# the page numbers of the format actually being produced.
TARGET = os.environ.get("REPORT_TARGET", "pdf")

from config import (BASE_IMAGES, DATA_DIR, FIG_DIR, RESULTS_DIR, SRC_DIR,
                    TABLE_DIR, VARIANTS)
...
# Printable width of an A4 page at the margins used by both renderers.
CONTENT_W_CM = 17.0

# Paragraph styles that both renderers understand.
STYLES = {
    "body":       dict(size=9.6, bold=False, italic=False, centre=False,
                       colour="#000000"),
    "small":      dict(size=8.6, bold=False, italic=False, centre=False,
                       colour="#000000"),
    "flow":       dict(size=10.0, bold=True, italic=False, centre=True,
                       colour="#000000"),
    "equation":   dict(size=9.6, bold=False, italic=False, centre=True,
                       colour="#000000", mono=True),
    "uni":        dict(size=19.0, bold=True, italic=False, centre=True,
                       colour="#12325c"),
    "faculty":    dict(size=11.0, bold=False, italic=False, centre=True,
                       colour="#333a44"),
    "courseline": dict(size=12.0, bold=True, italic=False, centre=True,
                       colour="#1d4d80"),
    "eyebrow":    dict(size=9.0, bold=True, italic=False, centre=True,
                       colour="#6a707a"),
    "covertitle": dict(size=23.0, bold=True, italic=False, centre=True,
                       colour="#12325c"),
    "coversub":   dict(size=13.0, bold=False, italic=False, centre=True,
                       colour="#1d4d80"),
    "abstract":   dict(size=10.5, bold=False, italic=True, centre=True,
                       colour="#333a44", indent_cm=1.5),
}
...
# ----------------------------------------------------------------------------
# The small authoring interface used by the content below
# ----------------------------------------------------------------------------
def p(text, style="body"):
    BLOCKS.append(("p", text, style))
...
def bullets(items):
    BLOCKS.append(("bullets", list(items)))


def figure(name, caption, max_height_cm=13.5):
    if (FIG_DIR / name).exists():
        BLOCKS.append(("figure", name, caption, max_height_cm))


def make_table(header, rows, col_widths=None, font_size=7.6, highlight_rows=()):
    BLOCKS.append(("table", list(header), [list(r) for r in rows],
                   list(col_widths) if col_widths else None, font_size,
                   tuple(highlight_rows)))
...
def code_heading(text):
    BLOCKS.append(("code_heading", text))
...
def rule(width_cm=None, colour="#1d4d80", thickness=1.1, space_before=6,
         space_after=6):
    BLOCKS.append(("rule", width_cm, colour, thickness, space_before,
                   space_after))


def read_table(name):
    with open(TABLE_DIR / name, newline="", encoding="utf-8") as fh:
...
p("COMPUTER VISION PROJECT", "eyebrow")
p(TITLE, "covertitle")
p("Thresholding, region growing and K means clustering", "coversub")
rule(width_cm=6, colour="#9aa6b4", thickness=0.8, space_before=14,
     space_after=16)

p("Segmentation of objects of interest from an image, followed by the "
  "computation of meaningful measurements about the detected objects, "
  "implemented in Python and OpenCV with classical computer vision only.",
  "abstract")
spacer(48)
...
], col_widths=[5.6, CONTENT_W_CM - 5.6], font_size=9.4)
...
# ============================================================================
h1("Contents")
# The two renderers paginate slightly differently, because Word places the
# figures on its own pages, so each one carries its own measured page numbers.
# The numbers were read back from the finished document of each format.
CONTENTS = [
    ["1", "Problem definition and objectives", {"pdf": "3", "docx": "3"}],
    ["2", "Dataset", {"pdf": "5", "docx": "4"}],
    ["3", "Methodology", {"pdf": "7", "docx": "6"}],
    ["4", "Implementation", {"pdf": "11", "docx": "9"}],
    ["5", "Experimental results", {"pdf": "12", "docx": "10"}],
    ["6", "Critical analysis", {"pdf": "24", "docx": "21"}],
    ["7", "Limitations and failure cases", {"pdf": "27", "docx": "23"}],
    ["8", "Conclusion", {"pdf": "29", "docx": "25"}],
    ["9", "References", {"pdf": "29", "docx": "25"}],
    ["A", "Complete source code", {"pdf": "31", "docx": "27"}],
]
make_table(["Section", "Title", "Page"],
           [[number, title, pages[TARGET]] for number, title, pages in CONTENTS],
           col_widths=[2.0, CONTENT_W_CM - 4.4, 2.4], font_size=9)
page_break()
...
make_table(["identifier", "content", "application domain", "objects", "licence"],
           rows, col_widths=[3.5, 3.9, 3.6, 1.7, 2.3],
           font_size=7.2)
...
figure("conditions_strip.png",
       ...
       "derived from it.", max_height_cm=6)
...
figure("ground_truth_overview.png",
       ...
       "green contour is one annotated region.", max_height_cm=6)
...
figure("otsu_histogram.png",
       ...
       "Otsu is the right tool.", max_height_cm=6.4)
...
make_table(["module", "responsibility"], [
    ...
    ["report_content.py", "The text, tables and figure list of this report, "
     "written once as neutral blocks."],
    ["make_report.py", "Renders those blocks into the PDF version of the "
     "report."],
    ["make_report_docx.py", "Renders the same blocks into the Word version, so "
     "the two files can never drift apart."],
], col_widths=[4.0, CONTENT_W_CM - 4.0], font_size=8)
table_caption("Table 2. Module map of the project.")
...
pre("cd src\n"
    ...
    "python make_report.py        # the PDF report\n"
    "python make_report_docx.py   # the Word version of the report")
spacer(6)
p("The complete run of the experiment driver takes about %s seconds on the "
  "machine used for this project." % SUMMARY.get("runtime_s", "400"))
...
make_table(["image", "method", "IoU", "precision", "recall", "F1",
           ...
           col_widths=[3.5, 3.4, 1.4, 1.6, 1.4,
                       1.4, 1.5, 1.7, 1.6],
           font_size=6.8)
...
make_table(["method", "mean IoU", "mean precision", "mean recall", "mean F1",
            "mean time (s)"], rows,
           col_widths=[5.2, 2.2, 2.6, 2.2, 2.0,
                       2.4], font_size=8, highlight_rows=best_index)
...
make_table(["method", "segmentation accuracy (mean IoU)",
           ...
           col_widths=[4.2, 3.6, 2.7, 3.2, 3.0],
...
make_table(["image", "threshold strategy", "IoU", "precision", "recall", "F1"],
           rows, col_widths=[4.2, 5.4, 1.7, 1.9, 1.7,
                             1.7], font_size=6.6)
...
figure("illumination_correction.png",
       ...
       "the image at once.", max_height_cm=5.2)
...
make_table(["image", "study", "setting", "seeds", "IoU", "recall",
            "regions", "time (s)"], rows,
           col_widths=[3.7, 2.5, 2.6, 1.5, 1.5,
                       1.5, 1.5, 1.7], font_size=6.6)
...
figure("region_growing_tolerance.png",
       ...
       "through the object boundary into the background.", max_height_cm=7)
figure("region_growing_seeds.png",
       ...
       "wide intensity range inside each object.", max_height_cm=7)
...
make_table(["image", "study", "setting", "IoU", "precision", "recall",
            "regions", "time (s)"], rows,
           col_widths=[3.7, 2.2, 2.4, 1.6, 1.8,
                       1.6, 1.5, 1.7], font_size=6.6)
...
figure("kmeans_k.png",
       ...
       "background to be separated from the coins.", max_height_cm=7)
...
make_table(["image", "filter", "IoU", "precision", "recall", "F1", "regions",
            "time (s)"], rows,
           col_widths=[4.4, 2.4, 1.7, 1.9, 1.7,
                       1.7, 1.5, 1.7], font_size=7)
...
figure("filter_study.png",
       ...
       "most of the damage.", max_height_cm=7)
...
make_table(["image", "condition", "method", "IoU", "F1", "regions"], rows,
           col_widths=[4.0, 3.6, 3.6, 1.6, 1.6,
                       1.6], font_size=6.6)
...
figure("conditions_coins_greek_07.png",
       "Figure 17. Robustness on the coins photograph.", max_height_cm=7)
...
make_table(["image", "objects present", "marker distance ratio",
           ...
           col_widths=[4.4, 2.6, 3.4, 3.2, 3.0],
           font_size=7)
...
make_table(["image", "objects present", "count before separation",
           ...
           col_widths=[5.4, 3.4, 3.9, 3.9], font_size=8)
...
make_table(["image", "method", "matched objects", "mean area error (%)",
           ...
           col_widths=[3.6, 3.3, 2.0, 2.2, 2.0,
                       2.2, 2.0], font_size=6.8)
...
for image_id in ("beans_coffee_06", "coins_greek_07"):
    figure("measure_%s.png" % image_id,
           ...
           "measurement on %s." % nice(image_id), max_height_cm=5.2)
figure("seed_area_histogram.png",
       ...
       "touching seeds that were not separated.", max_height_cm=6.4)
...
], col_widths=[3.0, 4.6, 4.6, 4.6], font_size=6.8)
...
figure("failure_dense.png",
       ...
       "orders of magnitude.", max_height_cm=5.6)
...
make_table(["identifier", "source", "licence"], rows,
           col_widths=[3.8, 9.4, 3.4], font_size=7)
...
p("The listing below is the complete source of the project, in the order in "
  ...
  "the src directory of the repository.", "small")
...
CODE_FILES = ["config.py", "dataset.py", "preprocessing.py", "segmentation.py",
              ...
              "make_report.py", "make_report_docx.py",
              "report_content.py"]
for name in CODE_FILES:
    path = SRC_DIR / name
    if not path.exists():
        continue
    code_heading("src/" + name)
